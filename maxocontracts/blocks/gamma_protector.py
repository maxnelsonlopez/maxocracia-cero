...
from maxocontracts.core.types import Participant, Wellness
...
class WellnessProtectorBlock:
    ...
    def check(self, participants: List[Participant]) -> WellnessCheckResult:
        ...
        all_ok = len(alerts) == 0 or all(
            a.level in [AlertLevel.FLOURISHING, AlertLevel.NEUTRAL] for a in alerts
        )

        return WellnessCheckResult(
            all_ok=all_ok, alerts=alerts, should_trigger_retraction=should_retract
        )
    ...
    def _evaluate_wellness(
        self, participant_id: str, wellness: Wellness
