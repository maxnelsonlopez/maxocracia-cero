...
class VHV:
    ...
    def __add__(self, other: "VHV") -> "VHV":
        ...
        return VHV(T=self.T + other.T, V=self.V + other.V, R=self.R + other.R)
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        return {"T": str(self.T), "V": str(self.V), "R": str(self.R)}
...
class SDV:
    ...
    def meets_minimum(self, actual: "SDV") -> bool:
        ...
        """
        return (
            actual.vivienda_m2 >= self.vivienda_m2
            and actual.alimentacion_kcal >= self.alimentacion_kcal
    ...
    def violations(self, actual: "SDV") -> Dict[str, str]:
        ...
        if actual.vivienda_m2 < self.vivienda_m2:
            violations["vivienda"] = (
                f"{actual.vivienda_m2}m² < {self.vivienda_m2}m² mínimo"
            )

        if actual.alimentacion_kcal < self.alimentacion_kcal:
            violations["alimentacion"] = (
                f"{actual.alimentacion_kcal}kcal < {self.alimentacion_kcal}kcal mínimo"
            )

        if actual.agua_litros_dia < self.agua_litros_dia:
            violations["agua"] = (
                f"{actual.agua_litros_dia}L < {self.agua_litros_dia}L mínimo"
            )
        ...
        if actual.trabajo_horas_semana_max > self.trabajo_horas_semana_max:
            violations["trabajo"] = (
                f"{actual.trabajo_horas_semana_max}h > {self.trabajo_horas_semana_max}h máximo"
            )
...
class SDV_S:
    ...
    def __post_init__(self):
        for dim in self.DIMENSIONS:
            ...
            if value < Decimal("0") or value > Decimal("1"):
                raise ValueError(
                    f"SDV-S '{dim}' debe estar en [0, 1], recibido {value}"
                )
    ...
    def meets_minimum(self, actual: "SDV_S") -> bool:
        ...
        return all(
            getattr(actual, dim) >= getattr(self, dim) for dim in self.DIMENSIONS
        )
    ...
    def violation_magnitude(self, actual: "SDV_S") -> Decimal:
        ...
        return sum(
            (deficits[dim] * weights[dim] for dim in self.DIMENSIONS), Decimal("0")
        )

    def suffering_factor(
        self, actual: "SDV_S", extra_magnitude: Decimal = Decimal("0")
    ) -> Decimal:
        """
...
class Participant:
    ...
    wellness_current: Wellness = field(
        default_factory=lambda: Wellness(value=Decimal("1.0"))
    )
    ...
    def update_wellness(self, new_value: Decimal) -> None:
        ...
        self.wellness_current = Wellness(value=new_value, participant_id=self.id)
    ...
    def is_in_good_standing(self, sdv_minimum: SDV) -> bool:
        ...
        return not self.wellness_current.is_suffering() and sdv_minimum.meets_minimum(
            self.sdv_actual
        )
...
class ContractTerm:
    ...
    assigned_participant: Optional[str] = (
        None  # Parte obligada (ej. 'user-1', 'synthetic-qwen-1')
    )

    def is_accepted_by_all(self, participant_ids: list) -> bool:
        """Verifica si todos los participantes aceptaron este término."""
        return all(self.accepted_by.get(pid, False) for pid in participant_ids)
