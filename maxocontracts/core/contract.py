...
from .types import SDV, VHV, ContractState, ContractTerm, Participant
...
class MaxoContract:
    ...
    def add_term(self, term: ContractTerm) -> None:
        ...
        self._log_event(
            "term_added", {"term_id": term.id, "description": term.description}
        )
    ...
    def accept_term(self, term_id: str, participant_id: str) -> bool:
        ...
        for term in self._terms:
            if term.id == term_id:
                ...
                self._log_event(
                    "term_accepted",
                    {"term_id": term_id, "participant_id": participant_id},
                )
                return True
        return False
    ...
    def all_terms_accepted(self) -> bool:
        ...
        return all(
            term.is_accepted_by_all(self.participant_ids) for term in self._terms
        )
    ...
    def submit_for_acceptance(self) -> bool:
        ...
        if not is_valid:
            self._log_event(
                "submission_rejected", {"reason": "axiom_validation_failed"}
            )
            return False
    ...
    def activate(self) -> bool:
        ...
        if not is_valid:
            self._log_event(
                "activation_rejected", {"reason": "axiom_validation_failed"}
            )
            return False
    ...
    def retract(self, reason: str, actor_id: str) -> bool:
        ...
        self._log_event(
            "contract_retracted",
            {"reason": reason, "actor_id": actor_id, "previous_state": previous_state},
        )
    ...
    def _log_event(
        self, event_type: str, data: Dict[str, Any], actor_id: Optional[str] = None
    ):
