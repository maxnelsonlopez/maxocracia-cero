...
class SyntheticOracle(OracleInterface):
    ...
    def _simulate_validation(
        self, query: OracleQuery, contract_data: Dict[str, Any]
    ) -> OracleResponse:
        ...
        verdict = Verdict(
            ...
            reasoning=(
                "; ".join(issues) if issues else "Contrato válido axiomáticamente"
            ),
        )
    ...
    def _simulate_retraction_eval(
        self, query: OracleQuery, reason: str, evidence: Dict[str, Any]
    ) -> OracleResponse:
        ...
        verdict = Verdict(
            approved=approved, confidence=confidence, reasoning=final_reason
        )
