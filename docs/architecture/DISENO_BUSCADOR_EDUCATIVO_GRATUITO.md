...
| **B4 — LLM juez + parlamento** ✅ (04-09-2026) | `ScoreEngine` local→OpenRouter free→nada; cola nocturna; `buscador_parameters` votable en el Parlamento Educativo (cooldown 14 días) | fail-open en 429/timeout, rúbrica JSON estricta, votación de pesos — **implementado: `app/score_engine.py` (stdlib, throttle 1.8s, rúbrica fija, `SinJuez`), cola `buscador_score_queue`, `mejor_score` (Nivel 2 refina), resoluciones vinculantes con cooldown (`resolver_parametro`, 409), insignia 🤖 en UI; 10 tests, plataforma 136/136, verificado en vivo (M15 403, 502 honesto sin juez)** |
| **B7 — Biblioteca privada** ✅ (04-09-2026, a petición de Max: sus PDFs de Springer en casa) | `ingest_biblioteca.py` (pypdf + txt/md, idempotente por hash, cifrado se omite, reporte total) + fragmentos FTS5 sin filtrar texto + federación sin lo privado | ingesta, idempotencia, fragmento-corto, cifrado-omitido — **implementado; plataforma 157/157, verificado en vivo** |
| **B8 — Biblioteca curada en 3 niveles** ✅ (04-09-2026, con DeepSeek: publicar para el bienestar sin redistribuir) | `catalogo_biblioteca.py` (inventario real: 478 ítems, hashes, OA-candidatos) + licencia/visibilidad/curaduría/categoría en esquema (con migración) + interruptor `POST /corpus/<id>/publicar` (M15) + searx por visibilidad | catálogo real, licencia explícita, cooldown de publicación, federación por niveles — **implementado; plataforma 160/160** |
| **B6 — La Lupa (Ojo Claro + Disenso)** ✅ (04-09-2026, a petición de Max: ver quién edita la verdad) | Meta-panorama del artículo (protección, reversiones, anonimato, guerra, editores) + diff palabra por palabra con difflib local; la máquina muestra, el humano juzga (M15) | panorama, diff de adjetivo retirado, fail-open — **implementado: `app/lupa.py` + 2 endpoints + UI 🔍 con timeline y visor; 10 tests, plataforma 146/146, verificado en vivo (Fotosíntesis: 14/30 reversiones, guerra=True, diff real)** |
| **B5 — Capas abiertas directas** ✅ (04-09-2026, adelantada a petición de Max: búsqueda general sin docker) | Wikipedia en español (referencia) + OpenAlex (academia con DOI/citas), APIs públicas sin clave, fail-open | parseo, fail-open, capas en la unificada — **implementado: `engine_wikipedia` + `engine_openalex` en `app/buscador.py`; 5 tests, verificado en vivo ("fotosintesis" → 10 académica + 5 referencia). Ampliación: motor genérico `engine_wikimedia` + Wikibooks/Wikiversidad, orden canónico semillas→corpus→referencia→académica→web con `orden_capas` auditable (P4); 7 tests, verificado en vivo** |
