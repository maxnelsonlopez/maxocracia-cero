...
- **Escuela invitada primero (26/9/2026, sesión con Max)**: la lectura ya era pública en backend (`buscador_routes.py:4-11`) pero el frontend la escondía tras el login — nueva `#guest-view` (héroe + buscador general y lupa al frente, login optativo solo para guardar progreso) en `plataforma_educativa/templates/index.html` + estados `showGuest/showAuth/showApp` en `static/app.js` + CSS mínimo; verificado `node --check` + 13 IDs cruzados.
