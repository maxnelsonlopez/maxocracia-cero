...
| Modelo de Tres Cuentas (CDD/CEH/TED) + γ doméstica + Red de Apoyo | 16 §16.3 + **16.5** (rama canónica: vector [T,V,R], CEH→TVI vendido, pesos p₁/p₂/p₃; §16.5.6 γ+INV1-Hogar; §16.5.12 Modo Escudo + ofertas de cuidado; §16.5.13 roommates/conjuntos; §16.5.14 Reino Natural conviviente) | `app/micromax_bp.py::/cdd`, `/dashboard`, `/checkin`, `/support/offers`; UI `frontend/app/micromax/page.tsx` | `tests/test_micromax.py` | `8edc059`, `6089308`–`da24f94` (segmentos), `ce44182` (Modo Escudo), `f9697a5` (compatibilidad), `a6844c8` (regenerativo), `8d48ba4` (γ), `bb8504c` (Red de Apoyo v1), `fbe403c` (UI canónica) |
| Semana de la Verdad — protocolo n=1 de validación humana | Cap. 15 §15.5 (métricas/aborto) + Informe Hallazgos v1.0 (hallazgos que la motivan) | `docs/guides/semana_de_la_verdad.md`; Guía acompañante `app/guide_bp.py::/chat` | — | docs (ago 2026) |
