...
```mermaid
graph TD
    A[Capa 5: Gobernanza] --> B[Oráculo Dinámico]
    A --> C[Asambleas de Calibración]
    B[Capa 4: Consenso] --> D[Blockchain Híbrida]
    D --> E[PoC + PoS]
    C[Capa 3: Datos] --> F[IoT + Wearables]
    F --> G[Satélites]
    F --> H[Sensores]
    C --> I[Bases de Datos Públicas]
    B[Capa 2: Cálculo] --> J[Motor VHV]
    J --> K[Simulador de Escenarios]
    A[Capa 1: Interfaz] --> L[Dashboards]
    L --> M[APIs]
    L --> N[App Móvil]
```
