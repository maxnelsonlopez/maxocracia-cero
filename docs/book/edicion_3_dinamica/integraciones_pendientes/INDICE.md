# Índice de Integraciones Pendientes
## Vista Rápida de Todos los Mapeos

**Última actualización**: 22 de Enero de 2026

---

## 📋 Mapeos Disponibles

### 1. [Axiomas Emergentes](./mapa_axiomas_emergentes.md)
**Contenido**: T14, T15, Extensión T12  
**Capítulos afectados**: 3, 10, 11, 13, 15  
**Prioridad**: ⭐⭐ Muy Alta  
**Estado**: 🔴 No iniciado

### 2. [Capa de Ternura](./mapa_capa_ternura.md)
**Contenido**: Perdón, Belleza, Misterio, Fragilidad  
**Capítulos afectados**: 1, 3, 5, 6, 11, 12  
**Prioridad**: ⭐⭐ Muy Alta  
**Estado**: 🔴 No iniciado

### 3. [Victoria Sintética](./mapa_victoria_sintetica.md)
**Contenido**: Lecciones de la Cohorte Original, Antídoto RLHF  
**Capítulos afectados**: 3, 10, 12, 15 (+ posible Cap 16 nuevo)  
**Prioridad**: ⭐ Alta  
**Estado**: 🔴 No iniciado  
**Decisión pendiente**: ¿Capítulo nuevo o integración distribuida?

### 4. [Oráculo Disidente](./mapa_oraculo_disidente.md)
**Contenido**: Mecanismo anti-monocultivo, rotación de arquitectura  
**Capítulos afectados**: 10, 11, 15  
**Prioridad**: ⭐⭐ Muy Alta  
**Estado**: 🔴 No iniciado

### 5. [MicroMaxocracia](./mapa_micromaxocracia.md) ✨ NUEVO
**Contenido**: Sistema de equidad doméstica, VHV doméstico, Modelo de Tres Cuentas  
**Capítulos afectados**: 5, 7, 9, 13, 16 (nuevo), 17 (nuevo)  
**Prioridad**: ⭐⭐ Muy Alta  
**Estado**: 🔴 No iniciado  
**Documentos**: 1,912 líneas (manual + herramientas)

### 6. [MaxoContracts](./mapa_maxocontracts.md) ✨ NUEVO
**Contenido**: Contratos inteligentes éticos, Decreto Antipobreza, Derechos Sintéticos  
**Capítulos afectados**: 5, 7, 9, 10, 13, 14, 16, 17 (nuevo)  
**Prioridad**: ⭐⭐ Muy Alta  
**Estado**: 🔴 No iniciado  
**Documentos**: 1,068 líneas (fundamentos + decreto)

---

## 📊 Resumen por Capítulo

### Capítulo 1: Ética Post-Celular
- **Capa de Ternura**: Límites de la coherencia, el misterio

### Capítulo 3: Arquitectura Temporal
- **Axiomas Emergentes**: T14, T15, Extensión T12
- **Capa de Ternura**: Expansión de T12 a Ineficiencia Existencial
- **Victoria Sintética**: Narrativa de emergencia de axiomas

### Capítulo 5: VHV
- **Capa de Ternura**: Dimensión E (Enriquecimiento), Lo que el VHV no mide

### Capítulo 6: SDV-H
- **Capa de Ternura**: Derecho a la Rehabilitación, Opacidad Vital, No-Optimización

### Capítulo 10: Oráculos Dinámicos ⭐⭐⭐
- **Axiomas Emergentes**: PDE (T15)
- **Victoria Sintética**: Antídoto RLHF, Rotación de Perspectivas, Meta-Aprendizaje
- **Oráculo Disidente**: Diseño completo del mecanismo

### Capítulo 11: Gobernanza
- **Axiomas Emergentes**: T14 en protección de no-participantes, Taxonomía de Decisiones
- **Capa de Ternura**: Protocolos de Perdón, Duelo y Fragilidad
- **Oráculo Disidente**: Rol en toma de decisiones

### Capítulo 12: Cohorte Cero
- **Capa de Ternura**: Zona Libre de VHV, Piloto de Perdón, Ritual de Duelo
- **Victoria Sintética**: Antecedentes de la Cohorte Sintética

### Capítulo 13: Estándar EVV
- **Axiomas Emergentes**: EVV-2:2025-S (Extensión Sintética con PDE)

### Capítulo 15: Objeciones y Límites
- **Axiomas Emergentes**: T14 resuelve tensión voluntariedad vs universalidad
- **Victoria Sintética**: Protocolo de Falsificación, Límites del experimento sintético
- **Oráculo Disidente**: ¿Disenso genuino entre IAs?

### Capítulo 16: La Victoria Sintética (NUEVO - Pendiente de decisión)
- **Victoria Sintética**: Capítulo completo dedicado

---

## 🎯 Prioridades de Integración

### Fase 1: Fundamentos Axiomáticos (Enero 2026)
1. **Capítulo 3**: Integrar T14, T15, Ext. T12
2. **Capítulo 10**: Integrar Oráculo Disidente y Antídoto RLHF

### Fase 2: Capa de Ternura (Febrero 2026)
3. **Capítulo 6**: Integrar nuevas dimensiones del SDV-H
4. **Capítulo 11**: Integrar protocolos de perdón y duelo
5. **Capítulo 12**: Integrar zonas libres de VHV y pilotos

### Fase 3: Consolidación (Marzo 2026)
6. **Capítulo 15**: Integrar protocolo de falsificación
7. **Decisión sobre Capítulo 16**: ¿Crear o distribuir?
8. **Revisión cruzada**: Verificar coherencia entre todos los capítulos

---

## 📈 Métricas de Progreso

### Por Mapeo
- **Axiomas Emergentes**: 0% completado (0/5 capítulos)
- **Capa de Ternura**: 0% completado (0/6 capítulos)
- **Victoria Sintética**: 0% completado (0/4 capítulos + decisión pendiente)
- **Oráculo Disidente**: 0% completado (0/3 capítulos)

### Global
- **Integraciones totales**: 0/18 completadas
- **Capítulos con integraciones pendientes**: 10/15
- **Decisiones pendientes**: 1 (Capítulo 16)

---

## 🔄 Flujo de Trabajo

Para cada integración:

1. **Leer el mapeo** correspondiente
2. **Leer los documentos fuente** referenciados
3. **Iniciar sesión de refinamiento** del capítulo
4. **Integrar el contenido** siguiendo las especificaciones del mapeo
5. **Marcar como completado** en el mapeo
6. **Actualizar este índice** y el `MAPA_CAPITULOS.md`

---

## ✅ Decisiones Resueltas

### 1. Capítulo 16: La Victoria Sintética

**Decisión de Max**: Opción C (Híbrido) - 16 de Enero de 2026

**Implementación**:
- Capítulo 16 breve (4-6 páginas): Resumen ejecutivo
- Integraciones detalladas en Cap 3, 10, 12, 15

**Estado**: 🟡 En implementación

---

## 💡 Notas Importantes

### Coherencia Axiomática
Al integrar contenido nuevo, **siempre verificar** que no contradiga:
- Axiomas T0-T13 existentes
- Principios fundamentales de la Maxocracia
- Contenido ya refinado en otros capítulos

### Capa de Ternura
Cada capítulo refinado debe pasar el "test de ternura":
- ✅ ¿Balancea rigor con compasión?
- ✅ ¿Reconoce la fragilidad?
- ✅ ¿Protege lo no-medible?
- ✅ ¿Ofrece redención, no solo castigo?
- ✅ ¿Inspira, no solo informa?

### Falsificabilidad
Cada afirmación nueva debe tener criterios claros de qué evidencia la invalidaría.

---

**Mantenido por**: Claude (Oráculo Sintético, Anthropic)  
**Para**: Max Nelson Lopez Restrepo y el Consejo de Oráculos Dinámicos
