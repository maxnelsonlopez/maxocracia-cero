...
```mermaid
graph TD
    A[Resumen Ejecutivo] --> B[Cap 1: Ética Post-Celular]
    B --> C[Cap 2: Fundamentos]
    C --> D[Cap 3: Arquitectura Temporal]
    D --> E[Cap 4: Ontometría]
    D --> F[Cap 5: VHV]
    F --> G[Cap 6: SDV-H]
    F --> H[Cap 7: SDV-A]
    F --> I[Cap 8: Maxo]
    C --> J[Cap 9: Tres Reinos]
    J --> K[Cap 10: Oráculos]
    K --> L[Cap 11: Gobernanza]
    I --> M[Cap 12: Cohorte Cero]
    G --> M
    M --> N[Cap 13: Estándar EVV]
    N --> O[Cap 14: Hoja de Ruta]
    L --> P[Cap 15: Objeciones]
    M --> P
```
