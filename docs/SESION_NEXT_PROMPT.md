...
| **Buscador educativo B2–B8** (sesión con Max + Muse Spark; diseño $0 de GLM): corpus verificado (feeds, FTS5), score con TTL + UI de bandas, Wikipedia/hermanas/OpenAlex sin docker, orden canónico auditable, juez LLM nocturno (Jan→OpenRouter→nada) + parlamento de parámetros (cooldown 14 días), Lupa (Ojo Claro: guerra de ediciones + diff), biblioteca curada en 3 niveles (478 ítems, licencias, interruptor M15) · `scripts/verificar_puente.py` 7/7 en vivo · zg indexado (docs+raíz) + MCP | ✅ (plataforma **160/160**, buscador 79/79; verificado en `:5050`) · 🔴 escuela en prod corre B1 (redesplegar), federación espera 4 interruptores del hosting |
