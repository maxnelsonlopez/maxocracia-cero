...
| **M12 Síntesis de Identidad del OEV** (29-08-2026: Gemini implementó · DeepSeek revisó/corrigió): **UNA sola puerta de identidad** entre :5001 y :5050 — autenticación híbrida en `plataforma_educativa/app/auth.py` (tokens locales en memoria + JWTs federados con clave compartida, JIT vinculando `maxo_user_id`, migración idempotente), blueprint `app/edu_bridge_bp.py` (`/status`, `/sync-mastery`, `/events`), doc `SINTESIS_IDENTIDAD_OEV.md` · **corrección del orquestador**: la v1 auto-promovía N0→N1 con un POST del propio usuario (la escalera se compraba) → ahora el nodo OEV reporta con token de servicio `EDU_BRIDGE_SERVICE_TOKEN` (fail-closed 403), **sin auto-promoción** (la escalera es del primer acuerdo, Cap. 13), `t13_hash` = SHA-256 real y `SECRET_KEY` sin constante pública (fail-closed 503) · **M12.1 puerta en la UI** (29-08): botón "Nodo Educativo" en la navegación (desktop + móvil) que redirige con el JWT en el **fragmento** de la URL (`#jwt=`, nunca viaja al servidor) y `app.js` del OEV lo captura como token federado · **celebrada EN VIVO** (29-08): demo end-to-end con usuario real (Max #1, 200/200/201/403/200 — paso a paso en `scratch/demo_m12.py`) | ✅ (40/40 en OEV, 5/5 puente, tsc limpio, demo en vivo ✅) |
...
**La rama educativa M1-M12 está CERRADA (29-08-2026)** — síntesis de identidad implementada, corregida, blindada y **celebrada en vivo**. Lo que sigue:

1. **Federar de verdad**: configurar en producción `SECRET_KEY` compartida + `EDU_BRIDGE_SERVICE_TOKEN` (ver README de la plataforma) — la demo usó valores de prueba; con la SSH/entorno real, misma clave en ambos nodos.
2. **Sincronización automática del OEV**: cuando el nodo apruebe maestrías por triada, debe reportarlas al puente con su token de servicio (hoy es una llamada manual desde el nodo; candidato: un endpoint en el OEV que reenvíe).
