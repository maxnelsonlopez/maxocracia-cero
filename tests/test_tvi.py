...
def tvi_manager(app):
    ...
    with app.app_context():
        ...
        pass
    return TVIManager(db_path=":memory:") # This won't work well because each connection is new DB
...
def db_path(tmp_path):
    ...
    # Create a user
    conn.execute("INSERT INTO users (email, password_hash, name) VALUES ('test@test.com', 'pass', 'Test User')")
    conn.commit()
    conn.close()
    return str(d)
...
def test_ccp_calculation(db_path):
    ...
    manager.log_tvi(user_id, "2025-01-01T00:00:00", "2025-01-01T08:00:00", "MAINTENANCE")
...
def test_api_endpoints(client, db_path, monkeypatch):
    ...
    auth_resp = client.post("/auth/register", json={
