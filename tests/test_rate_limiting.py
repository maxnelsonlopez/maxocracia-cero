...
def test_login_rate_limit(client):
    ...
    for i in range(150):
        resp = client.post(
            "/auth/login",
            json={"email": "rate_test@example.com", "password": "Password1"},
        )
...
def test_refresh_rate_limit(client):
    ...
    assert (
        "." in refresh_token
    ), "El token de refresco no tiene el formato esperado (jti.raw_token)"
    ...
    assert (
        new_refresh_token1
    ), "No se recibió nuevo token de refresco en la primera respuesta"
    ...
    assert (
        new_refresh_token2
    ), "No se recibió nuevo token de refresco en la segunda respuesta"
    ...
    assert (
        new_refresh_token3
    ), "No se recibió nuevo token de refresco en la tercera respuesta"
