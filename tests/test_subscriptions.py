...
class TestFairPriceCalculation:
    ...
    def test_ppp_adjustment_colombia(self, client):
        ...
        resp = client.post(
            "/subscriptions/calculate-fair-price", json={"country_code": "CO"}
        )
        assert resp.status_code == 200
    ...
    def test_ppp_adjustment_argentina(self, client):
        ...
        resp = client.post(
            "/subscriptions/calculate-fair-price", json={"country_code": "AR"}
        )
        data = resp.get_json()
    ...
    def test_ppp_adjustment_usa(self, client):
        ...
        resp = client.post(
            "/subscriptions/calculate-fair-price", json={"country_code": "US"}
        )
        data = resp.get_json()
    ...
    def test_income_adjustment_low_income(self, client):
        ...
        resp = client.post(
            "/subscriptions/calculate-fair-price",
            json={"country_code": "CO", "reported_monthly_income_usd": 400},
        )
        data = resp.get_json()
...
class TestPremiumRequiredDecorator:
    ...
    def test_free_user_cannot_access_premium(self, client, auth):
        ...
        client.get("/premium/dashboard", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_premium_response_structure(self, client, auth, app):
        ...
        resp = client.get("/test-premium", headers={"Authorization": f"Bearer {token}"})
...
class TestManualActivation:
    ...
    def test_only_admin_can_activate(self, client, auth):
        ...
        resp = client.post(
            "/subscriptions/activate-manual",
            headers={"Authorization": f"Bearer {token}"},
            json={"user_id": 1, "tier": "contributor", "months": 1},
        )
        assert resp.status_code == 403
