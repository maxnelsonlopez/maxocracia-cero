...
def set_level(client, uid, level, companion=None):
    ...
    res = client.post('/protection/profile', headers=user_headers(client, uid), json=body)
...
class TestProfile:
    ...
    def test_invalid_level_rejected(self, client, auth_header):
        res = client.post('/protection/profile', headers=auth_header,
...
class TestShielded:
    ...
    def test_witness_required_for_activation(self, client, auth_header, monkeypatch):
        monkeypatch.setattr('app.contracts_bp.LiveOracle', FakeOracle)
        set_level(client, 1, 'shielded')
        create_contract(client, auth_header, 'ctr-sh-3', [
            {'user_id': 1}, {'user_id': 2},
        ], min_reflection_hours=72)
        backdate_contract(client, 'ctr-sh-3', days=4)
        client.post('/contracts/ctr-sh-3/accept', headers=user_headers(client, 1),
                    json={'term_id': 'term-1', 'user_id': 1,
                          'comprehension': True, 'paraphrase': 'Prometo cumplir este acuerdo'})
        client.post('/contracts/ctr-sh-3/accept', headers=user_headers(client, 2),
        ...
        res = client.post('/contracts/ctr-sh-3/witness', headers=user_headers(client, 2))
        assert res.status_code == 400

        # user-3 (ajeno a las partes) es testigo
        res = client.post('/contracts/ctr-sh-3/witness', headers=user_headers(client, 3))
...
class TestComprehension:
    def test_assisted_sign_requires_paraphrase(self, client, auth_header, monkeypatch):
        ...
        create_contract(client, auth_header, 'ctr-cp-1', [{'user_id': 1}, {'user_id': 2}], min_reflection_hours=24)
        backdate_contract(client, 'ctr-cp-1', days=2)

        # Sin paráfrasis -> 400
        res = client.post('/contracts/ctr-cp-1/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1})
        assert res.status_code == 400
    ...
    def test_assisted_sign_blocked_without_live_oracle(self, client, auth_header):
        ...
        set_level(client, 1, 'assisted')
        create_contract(client, auth_header, 'ctr-cp-2', [{'user_id': 1}, {'user_id': 2}], min_reflection_hours=24)
        backdate_contract(client, 'ctr-cp-2', days=2)
        res = client.post('/contracts/ctr-cp-2/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1,
                                'comprehension': True,
                                'paraphrase': 'Entiendo que doy dos horas de mi tiempo'})
        assert res.status_code == 503
    ...
    def test_protected_delegate_requires_paraphrase(self, client, auth_header, monkeypatch):
        monkeypatch.setattr('app.contracts_bp.LiveOracle', FakeOracle)
        set_level(client, 1, 'assisted')
        res = client.post('/parties/', headers=auth_header, json={
            'party_id': 'coop-7', 'party_type': 'cooperative', 'display_name': 'Coop',
            'members': {'delegates': ['user-1', 'user-2'], 'quorum': 1.0},
        })
        assert res.status_code == 201
        create_contract(client, auth_header, 'ctr-cp-3', [
        ...
        res = client.post('/contracts/ctr-cp-3/accept', headers=auth_header,
...
class TestExposureCaps:
    def test_shielded_contract_cap(self, client, auth_header, monkeypatch):
        ...
        res = create_contract(client, auth_header, 'ctr-cap-1', [
            {'user_id': 1}, {'user_id': 2},
        ], terms=[
        ...
        res = create_contract(client, auth_header, 'ctr-cap-2', [
            {'user_id': 1}, {'user_id': 2},
        ], terms=[
            {'term_id': 't1', 'civil_text': 'Siete horas de trabajo',
             'vhv': {'t': 7.0, 'v': 0, 'h': 0}, 'assigned_participant_id': 'user-1'},
        ])
        assert res.status_code == 201

    def test_assisted_weekly_cap(self, client, auth_header):
        set_level(client, 1, 'assisted')  # tope semanal 40h
        create_contract(client, auth_header, 'ctr-cap-3', [
            {'user_id': 1}, {'user_id': 2},
        ], terms=[
            {'term_id': 't1', 'civil_text': 'Veinte horas de trabajo',
             'vhv': {'t': 20.0, 'v': 0, 'h': 0}, 'assigned_participant_id': 'user-1'},
        ])
        # Segundo contrato: 25h más -> 45h > 40h
        res = create_contract(client, auth_header, 'ctr-cap-4', [
            {'user_id': 1}, {'user_id': 2},
        ], terms=[
            {'term_id': 't1', 'civil_text': 'Veinticinco horas de trabajo',
             'vhv': {'t': 25.0, 'v': 0, 'h': 0}, 'assigned_participant_id': 'user-1'},
        ])
        assert res.status_code == 400
        assert res.get_json()['code'] == 'PROTECTION_CAP_EXCEEDED'
...
class TestReflectionFloor:
    def test_assisted_floor_enforced(self, client, auth_header):
        ...
        res = create_contract(client, auth_header, 'ctr-rf-1', [{'user_id': 1}],
                              min_reflection_hours=0)
        assert res.status_code == 400
        assert res.get_json()['code'] == 'PROTECTION_REFLECTION_FLOOR'

        res = create_contract(client, auth_header, 'ctr-rf-2', [{'user_id': 1}],
                              min_reflection_hours=30)
        assert res.status_code == 201
