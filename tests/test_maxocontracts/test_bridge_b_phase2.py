...
def test_full_cycle_need_to_active(client):
    ...
    assert all(
        t["accepted_by"].get("user-1") and t["accepted_by"].get("user-2")
...
def test_cycle_protected_requires_oracle(client):
    ...
    client.post(
        "/protection/profile", headers=auth(client, 1), json={"level": "assisted"}
    )
