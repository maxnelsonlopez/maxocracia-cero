...
def client():
    ...
    with app.test_client() as client:
        with app.app_context():
            ...
            db = get_db()
            db.execute(
                "INSERT INTO users (id, email, name) VALUES (1, 'max@example.com', 'Max')"
            )
            db.commit()
        yield client
...
def test_contract_persistence_cycle(client):
    ...
    )  # Nota: esto asume que el token_required es mockeado o manejado
...
def test_internal_persistence_functions(client):
    ...
    from app.contracts_bp import _load_contract, _save_contract
    from maxocontracts.core.contract import MaxoContract
    from maxocontracts.core.types import VHV, ContractTerm, Participant
