...
class TestLiquidDelegation:
    TERMS = [
        {
            "term_id": "term-a",
            "civil_text": "Cláusula A",
            "vhv": {"t": 2.0, "v": 0, "h": 2.0},
        },
        {
            "term_id": "term-b",
            "civil_text": "Cláusula B",
            "vhv": {"t": 3.0, "v": 0, "h": 3.0},
        },
    ]
    ...
    def test_expired_delegation_ignored(self, client, auth_header):
        make_party(
            ...
            members={
                ...
                "quorum": 1.0,
                "delegations": {
                    "user-1": {"proxy": "user-2", "valid_until": "2020-01-01T00:00:00"}
                },
            },
        )
        create_contract(
    ...
    def test_future_delegation_still_active(self, client, auth_header):
        make_party(
            ...
            members={
                ...
                "quorum": 1.0,
                "delegations": {
                    "user-1": {"proxy": "user-2", "valid_until": "2099-01-01T00:00:00"}
                },
            },
        )
        create_contract(
...
class TestQuorumLifecycle:
    def test_expired_window_rejects_and_extension_reopens(self, client, auth_header):
        ...
        res = client.post(
            "/parties/coop-7/quorum-extension",
            headers=auth_header,
