...
TERMS = [
    {
        "term_id": "do",
        "civil_text": "Max ofrece dos horas",
        "vhv": {"t": 2.0, "v": 0, "h": 2.0},
    ...
    {
        "term_id": "give",
        "civil_text": "Ana ofrece dos horas",
        "vhv": {"t": 2.0, "v": 0, "h": 2.0},
...
def report(client, contract_id, term_id, status, uid, evidence="Evidencia de prueba"):
    return client.post(
        f"/contracts/{contract_id}/terms/{term_id}/fulfillment",
        headers=user_headers(client, uid),
...
class TestPenalty:
    def test_penalty_gamma_validation(self, client, auth_header):
        res = client.post(
            ...
            headers=auth_header,
            json={
                "contract_id": "ctr-pen-0",
                "civil_description": "x",
                "participants": [{"user_id": 1}],
                "terms": [
                    {
                        "term_id": "t1",
                        "civil_text": "Hora de trabajo",
                        "vhv": {"t": 1.0, "v": 0, "h": 0},
                        "assigned_participant_id": "user-1",
                        "penalty_gamma": 1.5,
                    }
    ...
    def test_fulfilled_no_penalty(self, client, auth_header):
        ...
        max_p = next(
            p for p in res.get_json()["participants_details"] if p["id"] == "user-1"
        )
...
class TestAppeal:
    def test_appeal_restores_gamma(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-app-1/terms/do/appeal",
            headers=user_headers(client, 1),
    ...
    def test_appeal_by_third_party_forbidden(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-app-2/terms/do/appeal",
    ...
    def test_appeal_without_penalty_rejected(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-app-3/terms/do/appeal",
            headers=user_headers(client, 1),
