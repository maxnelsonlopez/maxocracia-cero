...
class TestAggregateWellness:
    def test_collective_wellness_is_members_mean(self, client, auth_header):
        ...
        coop = next(
            d for d in res.get_json()["participants_details"] if d["id"] == "coop-7"
        )
...
class TestHierarchy:
    def test_tree_endpoint_nested(self, client, auth_header):
        ...
        assert (
            tree["subcontracts"][0]["subcontracts"][0]["contract_id"] == "micro-nieto"
        )
    ...
    def test_subcontract_endpoint(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/no-existe/subcontracts",
            headers=auth_header,
