...
class TestCollectivePartyInContracts:
    ...
    def test_add_participant_with_party_id(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-escalas-3/participants",
            headers=auth_header,
            json={"party_id": "society-3"},
        )
    ...
    def test_invalid_party_id_rejected(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-escalas-4/participants",
            headers=auth_header,
            json={"party_id": "banana-1"},
        )
        assert res.status_code == 400
    ...
    def test_collective_wellness_syncs_to_registry(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-escalas-5/participants",
            headers=auth_header,
            json={"party_id": "coop-7", "wellness": 0.8},
        )
        assert res.status_code == 200
...
class TestContractReload:
    ...
    def test_add_participant_to_pending_contract_rejected(self, client, auth_header):
        ...
        res = client.post(
            "/contracts/ctr-reload-2/participants",
            headers=auth_header,
            json={"user_id": 3},
        )
        assert res.status_code == 400
