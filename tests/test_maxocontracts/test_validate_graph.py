    ...
    payload = {
        "nodes": [
            {"id": "start", "type": "input", "data": {"label": "Start"}},
            {
                "id": "action-1",
                "type": "action",
                "data": {"label": "Lavar platos", "vhvCost": 1.5},
            },
...
def test_validate_graph_invalid_axiom_t9(client, auth_header):
    ...
    payload = {
        ...
        "edges": [{"source": "start", "target": "action-1"}],
        "duration": 30,
    }
    ...
    t9_failures = [
        r for r in data["results"] if r["axiom"] == "T17" and not r["is_valid"]
    ]
    assert len(t9_failures) == 1
    assert (
        "no está conectada a ningún bloque de Reciprocidad" in t9_failures[0]["message"]
    )
    ...
    payload = {
        "nodes": [
            ...
            {"id": "cond-3", "type": "condition", "data": {"label": "Cond 3"}},
            {
                "id": "action-1",
                "type": "action",
                "data": {"label": "Super Acción", "vhvCost": 10.0},
            },
            {
                "id": "recip-1",
                "type": "reciprocity",
                "data": {"label": "Súper Reciprocidad"},
            },
        ],
        "edges": [
