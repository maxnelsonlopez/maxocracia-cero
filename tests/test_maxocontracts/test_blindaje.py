...
def create_contract(client, headers, contract_id, participants, terms=None, **extra):
    return client.post('/contracts/', headers=headers, json={
        'contract_id': contract_id,
        'civil_description': 'Contrato de blindaje',
        'participants': participants,
        'terms': terms or [
            {'term_id': 'term-1', 'civil_text': 'Acción balanceada',
             'vhv': {'t': 2.0, 'v': 0, 'h': 2.0}},
        ],
        **extra,
    })
...
class TestIdentity:
    def test_sign_as_other_user_rejected(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-id-1', [{'user_id': 1}, {'user_id': 2}])
        res = client.post('/contracts/ctr-id-1/accept', headers=auth_header,
    ...
    def test_sign_own_term_ok(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-id-2', [{'user_id': 1}, {'user_id': 2}])
        res = client.post('/contracts/ctr-id-2/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1})
        assert res.status_code == 200

    def test_delegate_spoofing_has_no_effect(self, client, auth_header):
        make_party(client, auth_header, 'coop-7', 'cooperative', 'Coop', members={
            'delegates': ['user-1', 'user-2'], 'quorum': 1.0,
        })
        create_contract(client, auth_header, 'ctr-id-3', [
        ...
        res = client.post('/contracts/ctr-id-3/accept', headers=auth_header,
    ...
    def test_nps_for_other_rejected(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-id-4', [{'user_id': 1}, {'user_id': 2}])
        res = client.post('/contracts/ctr-id-4/nps', headers=auth_header,
...
class TestImmutability:
    def test_recreate_active_contract_conflict(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-imm-1', [{'user_id': 1}, {'user_id': 2}])
        for uid in (1, 2):
            client.post('/contracts/ctr-imm-1/accept', headers=user_headers(client, uid),
                        json={'term_id': 'term-1', 'user_id': uid})
        assert client.post('/contracts/ctr-imm-1/activate', headers=auth_header).status_code == 200
    ...
    def test_recreate_others_draft_conflict(self, client, auth_header):
        ...
        res = create_contract(client, user_headers(client, 2), 'ctr-imm-2', [{'user_id': 2}])
        assert res.status_code == 409
...
class TestPartyAuthority:
    def test_non_owner_update_forbidden(self, client, auth_header):
        make_party(client, auth_header, 'coop-7', 'cooperative', 'Coop de Max', members={
            'delegates': ['user-1', 'user-2'], 'quorum': 1.0,
        })
        # user-3 intenta secuestrar la gobernanza
        res = client.put('/parties/coop-7', headers=user_headers(client, 3), json={
    ...
    def test_owner_update_ok(self, client, auth_header):
        ...
        res = client.put('/parties/coop-7', headers=auth_header, json={
    ...
    def test_delegate_quorum_governance_change(self, client, auth_header):
        make_party(client, auth_header, 'coop-7', 'cooperative', 'Coop de Max', members={
            'delegates': ['user-1', 'user-2', 'user-3'], 'quorum': 0.6,
        })
        proposal = {'delegates': ['user-2', 'user-3', 'user-4'], 'quorum': 0.6}

        # user-2 vota
        res = client.post('/parties/coop-7/governance-change', headers=user_headers(client, 2),
        ...
        res = client.post('/parties/coop-7/governance-change', headers=user_headers(client, 4),
                          json={'members': proposal})
        assert res.status_code == 403

        # user-3 vota -> quórum (2 de 3) -> se aplica
        res = client.post('/parties/coop-7/governance-change', headers=user_headers(client, 3),
                          json={'members': proposal})
        assert res.status_code == 200
        assert res.get_json()['applied'] is True

        res = client.get('/parties/coop-7', headers=auth_header)
        assert res.get_json()['party']['members']['delegates'] == ['user-2', 'user-3', 'user-4']

    def test_non_owner_delete_forbidden(self, client, auth_header):
        make_party(client, auth_header, 'coop-7', 'cooperative', 'Coop de Max', members={
            'delegates': ['user-1'], 'quorum': 1.0,
        })
...
class TestWellnessSource:
    def test_wellness_out_of_range_rejected(self, client, auth_header):
        res = create_contract(client, auth_header, 'ctr-g-1',
                              [{'user_id': 1, 'wellness': 2.0}])
        assert res.status_code == 400
        assert 'rango' in res.get_json()['error']

        create_contract(client, auth_header, 'ctr-g-2', [{'user_id': 1}])
        res = client.post('/contracts/ctr-g-2/participants', headers=auth_header,
                          json={'user_id': 2, 'wellness': 0.4})
        assert res.status_code == 400

    def test_wellness_reported_by_recorded(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-g-3', [{'user_id': 1, 'wellness': 1.2}])
...
class TestAsymmetry:
    def _imbalanced(self, client, auth_header, contract_id):
        ...
        return create_contract(client, auth_header, contract_id, [
            {'user_id': 1}, {'user_id': 2},
        ], terms=[
            {'term_id': 'do', 'civil_text': 'Max ofrece diez horas',
             'vhv': {'t': 10.0, 'v': 0, 'h': 0}, 'assigned_participant_id': 'user-1'},
            {'term_id': 'give', 'civil_text': 'Ana ofrece tres horas',
             'vhv': {'t': 3.0, 'v': 0, 'h': 0}, 'assigned_participant_id': 'user-2'},
    ...
    def test_activation_blocked_without_acknowledgment(self, client, auth_header):
        ...
        for uid in (1, 2):
            for term in ('do', 'give'):
                client.post('/contracts/ctr-asym-2/accept', headers=user_headers(client, uid),
    ...
    def test_activation_after_full_acknowledgment(self, client, auth_header):
        ...
        for uid in (1, 2):
            for term in ('do', 'give'):
                client.post('/contracts/ctr-asym-3/accept', headers=user_headers(client, uid),
                            json={'term_id': term, 'user_id': uid})
            res = client.post('/contracts/ctr-asym-3/acknowledge-asymmetry',
...
class TestProhibitedClauses:
    def test_retraction_waiver_rejected(self, client, auth_header):
        res = create_contract(client, auth_header, 'ctr-proh-1', [{'user_id': 1}], terms=[
            {'term_id': 't1', 'civil_text': 'Sin derecho a retractarse nunca',
             'vhv': {'t': 1.0, 'v': 0, 'h': 0}},
        ])
        assert res.status_code == 400
    ...
    def test_exclusivity_rejected_on_add_term(self, client, auth_header):
        ...
        res = client.post('/contracts/ctr-proh-2/terms', headers=auth_header, json={
            'term_id': 't1', 'civil_text': 'Exclusividad total con el taller',
            'vhv': {'t': 1.0, 'v': 0, 'h': 0},
        })
        assert res.status_code == 400
    ...
    def test_legalese_rejected(self, client, auth_header):
        ...
        res = create_contract(client, auth_header, 'ctr-proh-3', [{'user_id': 1}], terms=[
            {'term_id': 't1', 'civil_text': long_text, 'vhv': {'t': 1.0, 'v': 0, 'h': 0}},
        ])
        assert res.status_code == 400
...
class TestUnassignedObligations:
    def test_unassigned_heavy_term_rejected(self, client, auth_header):
        res = create_contract(client, auth_header, 'ctr-ob-1', [{'user_id': 1}], terms=[
            {'term_id': 't1', 'civil_text': 'Diez horas de trabajo',
             'vhv': {'t': 10.0, 'v': 0, 'h': 0}},  # sin assigned_participant_id
        ])
        assert res.status_code == 400
...
class TestTimeWindows:
    def test_signature_deadline_expired(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-win-1', [{'user_id': 1}],
                        signature_deadline='2020-01-01T00:00:00')
        res = client.post('/contracts/ctr-win-1/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1})
    ...
    def test_reflection_window_blocks_signing(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-win-2', [{'user_id': 1}],
                        min_reflection_hours=24)
        res = client.post('/contracts/ctr-win-2/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1})
        assert res.status_code == 423
        assert res.get_json()['code'] == 'REFLECTION_PENDING'

    def test_reflection_zero_allows_immediate(self, client, auth_header):
        create_contract(client, auth_header, 'ctr-win-3', [{'user_id': 1}],
                        min_reflection_hours=0)
        res = client.post('/contracts/ctr-win-3/accept', headers=auth_header,
                          json={'term_id': 'term-1', 'user_id': 1})
        assert res.status_code == 200
