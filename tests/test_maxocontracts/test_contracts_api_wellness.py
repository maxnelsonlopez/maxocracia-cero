...
def test_add_participant_wellness_parameter(client, auth_header):
    ...
    res = client.post(
        "/contracts/",
        headers=auth_header,
        json={"contract_id": "contract-wellness", "civil_description": "Test Contract"},
    )
    assert res.status_code == 201

    # 2. Add participant using 'wellness'
    res = client.post(
        "/contracts/contract-wellness/participants",
        headers=auth_header,
        json={"user_id": 2, "wellness": 1.5},
    )
...
def test_add_participant_gamma_legacy(client, auth_header):
    ...
    res = client.post(
        "/contracts/contract-gamma/participants",
        headers=auth_header,
        json={"user_id": 2, "gamma": 1.2},
    )
