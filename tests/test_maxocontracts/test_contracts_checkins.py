...
def _create_ready_contract(client):
    ...
    assert _add_participant(client, h, 'checkin-contract', 1, wellness=1.0).status_code == 200
    assert _add_participant(client, h, 'checkin-contract', 2, wellness=1.0).status_code == 200
    return h
...
def test_checkin_defaults_to_token_actor(client):
    ...
    assert _create_contract(client, h, 'checkin-self', description='Mi contrato').status_code == 201
    assert _add_participant(client, h, 'checkin-self', 2, wellness=1.0).status_code == 200

    res = client.post('/contracts/checkin-self/checkin', headers=h, json={'wellness': 1.1})
    assert res.status_code == 201
    data = res.get_json()
    assert data['participant_id'] == 'user-2'
...
def test_checkin_updates_contract_gamma(client):
    ...
    client.post('/contracts/checkin-contract/checkin', headers=h, json={
        'wellness': 0.65,
        'participant_id': 'user-2',
    })
...
def test_checkin_unknown_contract(client):
    h = user_headers(client, 1)
    res = client.post('/contracts/does-not-exist/checkin', headers=h, json={'wellness': 1.0})
...
def test_cohort_gamma_uses_real_checkins(client):
    ...
    assert _create_contract(client, h, 'coop-a-1', description='Contrato coop').status_code == 201
    res = client.post('/contracts/coop-a-1/participants', headers=h, json={
        'party_id': 'coop-7',
    ...
    assert _create_contract(client, h, 'coop-a-2', description='Segundo contrato coop').status_code == 201
    res = client.post('/contracts/coop-a-2/participants', headers=h, json={
        'party_id': 'coop-7',
        'party_type': 'cooperative',
        'display_name': 'Cooperativa Semilla',
    })
    ...
    assert client.post('/contracts/coop-a-1/checkin', headers=h, json={
        'wellness': 1.2,
        'participant_id': 'coop-7',
    }).status_code == 201
    assert client.post('/contracts/coop-a-2/checkin', headers=h, json={
        'wellness': 1.0,
        'participant_id': 'coop-7',
    }).status_code == 201
