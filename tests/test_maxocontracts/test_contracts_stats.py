...
def test_stats_sdv_violations(client, auth_header):
    ...
    res = client.post(
        "/contracts/c-sdv-1/participants",
        headers=auth_header,
        json={
...
def test_stats_nps_and_recording(client, auth_header):
    ...
    assert (
        client.post(
            "/contracts/c-nps-1/nps",
            headers=user_headers(client, 1),
            json={"participant_id": "user-1", "score": 9, "comment": "Excelente"},
        ).status_code
        == 201
    )
    assert (
        client.post(
            ...
            json={"participant_id": "user-2", "score": 10},
        ).status_code
        == 201
    )
    assert (
        client.post(
            ...
            json={"participant_id": "user-3", "score": 5},
        ).status_code
        == 201
    )
...
def test_nps_validation(client, auth_header):
    ...
    res = client.post(
        "/contracts/c-nps-2/nps",
        headers=auth_header,
        json={"participant_id": "user-1", "score": 11},
    )
    ...
    res = client.post(
        "/contracts/c-nps-2/nps",
        headers=auth_header,
        json={"participant_id": "user-1", "score": "nueve"},
    )
    assert res.status_code == 400
    ...
    res = client.post(
        "/contracts/c-nps-2/nps",
        headers=auth_header,
        json={"participant_id": "user-2", "score": 8},
    )
    assert res.status_code == 400

    # Contrato inexistente
    res = client.post(
        "/contracts/no-existe/nps",
        headers=auth_header,
        json={"participant_id": "user-1", "score": 8},
    )
    ...
    client.post(
        "/contracts/c-nps-2/nps",
        headers=auth_header,
        json={"participant_id": "user-1", "score": 7},
    )
    res = client.post(
        "/contracts/c-nps-2/nps",
        headers=auth_header,
...
def test_stats_categories_and_vhv(client, auth_header):
    ...
    for cid, cat in [
        ...
        res = client.post(
            f"/contracts/{cid}/meta",
            headers=auth_header,
            json={"key": "category", "value": cat},
        )
        assert res.status_code == 200

    # Añadir VHV a uno de los contratos
    res = client.post(
        "/contracts/c-cat-1/terms",
        headers=auth_header,
        json={
            "term_id": "limpieza-1",
