...
def _propose(client, params, uid=1, **extra):
    return client.post('/voting/parliament/params', headers=auth(client, uid), json={
        **params, **extra,
    })


def _close_proposal(client, proposal_id, uid=1):
    return client.post(f'/voting/proposals/{proposal_id}/close', headers=auth(client, uid))
...
def test_propose_valid_params(client):
    ...
    res = _propose(client, {'alpha': 120.0, 'beta': 2500.0, 'gamma': 1.5, 'delta': 80.0})
...
def test_approved_proposal_applies_params(client):
    ...
    res = _propose(client, {'alpha': 150.0, 'beta': 3000.0, 'gamma': 1.2, 'delta': 50.0})
    prop_id = res.get_json()['proposal']['id']

    # 3 de 4 usuarios votan Aprobar (quórum 0.75 ≥ 0.6; mayoría 1.0 ≥ 0.75)
    for uid in (1, 2, 3):
        assert client.post(f'/voting/proposals/{prop_id}/vote', headers=auth(client, uid), json={
            'option': 'Aprobar',
        }).status_code == 200
    ...
    assert _params_in_db(client) == {'alpha': 150.0, 'beta': 3000.0, 'gamma': 1.2, 'delta': 50.0}
...
def test_rejected_proposal_no_change(client):
    ...
    res = _propose(client, {'alpha': 500.0, 'beta': 2000.0, 'gamma': 1.0, 'delta': 100.0})
    prop_id = res.get_json()['proposal']['id']

    # 3 votos: 2 Aprobar, 1 Mantener → mayoría 2/3 = 0.667 < 0.75 → rechazada
    for uid, opt in ((1, 'Aprobar'), (2, 'Aprobar'), (3, 'Mantener')):
        client.post(f'/voting/proposals/{prop_id}/vote', headers=auth(client, uid), json={'option': opt})
    payload = _close_proposal(client, prop_id).get_json()['proposal']
...
def test_quorum_not_met_no_change(client):
    ...
    before = _params_in_db(client)
    res = _propose(client, {'alpha': 999.0, 'beta': 2000.0, 'gamma': 1.0, 'delta': 100.0})
    prop_id = res.get_json()['proposal']['id']
    client.post(f'/voting/proposals/{prop_id}/vote', headers=auth(client, 1), json={'option': 'Aprobar'})
    payload = _close_proposal(client, prop_id).get_json()['proposal']
...
def test_parliament_public_view(client):
    """GET público: actuales + historial + propuestas abiertas (T13)."""
    res = _propose(client, {'alpha': 110.0, 'beta': 2000.0, 'gamma': 1.0, 'delta': 100.0})
    prop_id = res.get_json()['proposal']['id']
    for uid in (1, 2, 3):
        client.post(f'/voting/proposals/{prop_id}/vote', headers=auth(client, uid), json={'option': 'Aprobar'})
...
def test_normal_proposals_unaffected(client):
    ...
    res = client.post('/voting/proposals', headers=auth(client), json={
    ...
    before = _params_in_db(client)
    for uid in (1, 2, 3):
        client.post(f'/voting/proposals/{prop_id}/vote', headers=auth(client, uid), json={'option': 'Sí'})
    payload = _close_proposal(client, prop_id).get_json()['proposal']
    assert payload['result'] == 'passed'
    assert _params_in_db(client) == before
