...
def _create_contract_with_terms(client, contract_id='plaza-1'):
    ...
    h = auth(client)
    assert client.post('/contracts/', headers=h, json={
        'contract_id': contract_id,
        'civil_description': 'Intercambio de la plaza',
    }).status_code == 201
    assert client.post(f'/contracts/{contract_id}/participants', headers=h, json={
        'user_id': 1, 'wellness': 1.0,
    }).status_code == 200
    assert client.post(f'/contracts/{contract_id}/participants', headers=h, json={
        'user_id': 2, 'wellness': 1.0,
    }).status_code == 200
    assert client.post(f'/contracts/{contract_id}/terms', headers=h, json={
        'term_id': 'term-1',
...
def test_verifier_hash_stable_across_states(client):
    ...
    for term in ('term-1', 'term-2'):
        for uid in (1, 2):
            assert client.post(f'/contracts/plaza-1/accept', headers=auth(client, uid), json={
                'term_id': term,
                'user_id': uid,
            }).status_code in (200, 201)
    assert client.post('/contracts/plaza-1/activate', headers=h).status_code in (200, 201)
...
def test_verifier_sanitized_no_personal_data(client):
    ...
    assert set(participant.keys()) == {
        'participant_id', 'party_type', 'is_collective', 'wellness',
        'checkins_count', 'last_checkin_wellness', 'last_checkin_at',
    }
...
def test_verifier_cohort_public(client):
    ...
    h = _create_contract_with_terms(client)
    assert client.post('/contracts/plaza-1/checkin', headers=h, json={
        'wellness': 0.95,
    ...
    assert abs(data['wellness']['avg'] - 0.975) < 1e-4  # (1.0 de Alice + 0.95 de Bob) / 2
