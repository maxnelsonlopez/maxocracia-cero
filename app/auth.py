...
def me(current_user):
    ...
    db = get_db()
    row = db.execute(
        "SELECT id, email, name, alias, phone, city, neighborhood, created_at FROM users WHERE id = ?",
        (user_id,),
    ).fetchone()
    if row is None:
...
def refresh():
    ...
    rotate_success = rotate_refresh_token(
        jti, new_jti, new_raw, expires_in=refresh_expires
    )
