...
def list_resources():
    db = get_db()
    cur = db.execute(
        "SELECT id, user_id, title, category, available, created_at FROM resources ORDER BY created_at DESC LIMIT 100"
    )
...
def create_resource():
    ...
    try:
        db.execute(
            "INSERT INTO resources (user_id, title, description, category, available) VALUES (?, ?, ?, ?, ?)",
...
def claim_resource(res_id):
    ...
    db.execute(
        'INSERT INTO interchange (interchange_id, date, giver_id, receiver_id, type, description) VALUES (?, DATE("now"), ?, ?, ?, ?)',
        (
            f"INT-{res_id}-{requester_id}",
            owner_id,
            requester_id,
            row["category"],
            row["description"],
