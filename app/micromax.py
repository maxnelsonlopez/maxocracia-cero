...
def init_micromax_tables(app):
    ...
    try:
        ...
        # Indices
        cur.execute("CREATE INDEX IF NOT EXISTS idx_micromax_members_household ON micromax_members(household_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_micromax_cdd_member ON micromax_cdd_logs(member_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_micromax_cdd_date ON micromax_cdd_logs(logged_date)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_micromax_audits_household ON micromax_audits(household_id)")
...
class MicroMaxManager:
    ...
    def create_household(self, name: str) -> Dict:
        ...
        for _ in range(10):  # Try 10 times to avoid collision
            ...
            cursor.execute("SELECT 1 FROM micromax_households WHERE invite_code = ?", (code,))
    ...
    def get_household_members(self, household_id: int) -> List[Dict]:
        ...
        conn = self._get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM micromax_members WHERE household_id = ?", (household_id,))
        return [dict(row) for row in cursor.fetchall()]
        
    def update_member_config(
        self, user_id: int, monthly_income: float, work_hours: float, travel_hours: float, sleep_hours: float
    ) -> Dict:
        ...
        if (work_hours + travel_hours + sleep_hours) > 168:
            raise ValueError("Total hours (work + travel + sleep) cannot exceed 168 hours in a week.")
    ...
    def log_cdd(
        ...
        cursor.execute("SELECT score FROM micromax_safety_surveys WHERE member_id = ?", (member["id"],))
        survey = cursor.fetchone()
        if survey and survey["score"] >= 3:
            raise ValueError("MicroMaxocracia Access Blocked: ESI Checklist score is too high (safety threshold crossed).")
        ...
        """, (
            member["id"], task_name.strip(), duration_hours, effort_factor, mental_factor, scope_factor,
            attention_factor, fragmentation_factor, loneliness_factor, calculated_vhv, logged_date
        ))
    ...
    def get_cdd_logs(self, user_id: int, limit: int = 50, offset: int = 0) -> List[Dict]:
    ...
    def get_safety_survey(self, user_id: int) -> Optional[Dict]:
        ...
        conn = self._get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM micromax_safety_surveys WHERE member_id = ?", (member["id"],))
        row = cursor.fetchone()
        if not row:
            return None
    ...
    def log_audit(
        ...
        """, (
            member["household_id"], audit_date, conflicts_count, weapon_count, accusations_count, threats_count,
            s1_hours, s2_score, s3_score, s4_score, s5_score, duration_weeks
        ))
    ...
    def calculate_three_accounts(self, household_id: int, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict:
        ...
        member_teds = {}
        for m in members:
            ted = 168.0 - m["work_hours"] - m["travel_hours"] - (m["sleep_hours"] or 56.0)
    ...
    def calculate_toxicity_indices(self, household_id: int) -> Dict:
        ...
        mean_cdd = sum(cdd_weekly_averages) / len(cdd_weekly_averages) if cdd_weekly_averages else 0.0
        ...
        reasons = []
        if ice_alert:
            reasons.append(f"ICE ({round(ice, 2)}) supera umbral 3.0 (Conflictos altos o uso coercitivo del ledger)")
        if idb_alert:
            reasons.append(f"IDB ({round(idb, 2)}) supera umbral 5.0 (Ansiedad o deterioro de bienestar)")
        if idp_alert:
            reasons.append(f"IDP ({round(idp, 2)}) indica desequilibrio doméstico persistente sin corregir")
