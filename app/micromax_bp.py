...
def create_household(current_user):
    ...
    try:
        ...
        db = get_db()
        user_row = db.execute("SELECT name, email FROM users WHERE id = ?", (current_user["user_id"],)).fetchone()
        user_name = user_row["name"] if (user_row and user_row["name"]) else (user_row["email"] if user_row else current_user.get("email") or "Usuario")

        # Create household
        household = manager.create_household(name)
        # Automatically join the creator as a member
        join_res = manager.join_household(household["invite_code"], current_user["user_id"], user_name)
        return jsonify({
            "household": household,
...
def get_household(current_user):
    try:
        ...
        db = get_db()
        cursor = db.cursor()
        cursor.execute("SELECT * FROM micromax_households WHERE id = ?", (member["household_id"],))
...
def update_member_config(current_user):
    ...
    try:
        ...
        member = manager.update_member_config(
            current_user["user_id"], monthly_income, work_hours, travel_hours, sleep_hours
        )
...
def log_cdd(current_user):
    ...
    required = ["task_name", "duration_hours", "effort_factor", "mental_factor", "scope_factor"]
    for req in required:
        if req not in data:
...
def log_audit(current_user):
    ...
    required = ["audit_date", "conflicts_count", "weapon_count", "accusations_count", "threats_count"]
    for req in required:
        if req not in data:
            return jsonify({"error": f"Falta el campo requerido: {req}"}), 400
...
def get_dashboard(current_user):
    try:
        member = manager.get_member(current_user["user_id"])
        if not member:
            return jsonify({"error": "No eres miembro de ningún hogar MicroMaxocracia."}), 404
            
        start_date = request.args.get("start_date")
        end_date = request.args.get("end_date")
        
        three_accounts = manager.calculate_three_accounts(member["household_id"], start_date, end_date)
