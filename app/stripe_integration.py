...
from .subscriptions import PPP_ADJUSTMENTS, PREMIUM_TIERS, get_db
...
def get_or_create_stripe_customer(user_id: int, email: str, name: str) -> str:
    ...
    result = db.execute(
        "SELECT external_customer_id FROM subscriptions WHERE user_id = ?", (user_id,)
    ).fetchone()
    ...
    customer = stripe.Customer.create(
        email=email,
        name=name,
        metadata={"user_id": str(user_id), "platform": "maxocracia"},
    )
    ...
    return jsonify(
        {
            ...
            "prices_configured": {
                tier: bool(price_id) for tier, price_id in STRIPE_PRICE_IDS.items()
            },
            "principles": {
                        ...
                        "products": [
                            {
                                "product": os.environ.get(
                                    "STRIPE_PRODUCT_CONTRIBUTOR", ""
                                ),
                                "prices": [
                                    os.environ.get("STRIPE_PRICE_CONTRIBUTOR", "")
                                ],
                            },
                            {
                                "product": os.environ.get(
...
def stripe_webhook():
    ...
    try:
        if event_type == "checkout.session.completed":
            ...
            current_app.logger.info(
                f"[STRIPE WEBHOOK] Evento no manejado: {event_type}"
            )
...
def get_stripe_subscription_status(current_user):
    ...
    try:
        stripe_sub = stripe.Subscription.retrieve(
            result["external_subscription_id"], expand=["latest_invoice.payment_intent"]
        )
        ...
        return jsonify(
            {
                ...
                "plan": {
                    ...
                    "amount": (
                        stripe_sub.plan.amount if hasattr(stripe_sub, "plan") else None
                    ),
                    "currency": (
                        stripe_sub.plan.currency
                        if hasattr(stripe_sub, "plan")
                        else None
                    ),
                },
            }
