...
export default function AdminVHVProducts() {
  ...
  const [editing, setEditing] = useState<VHVProduct | null>(null);
  const [form, setForm] = useState({ name: "", category: "", description: "" });
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState<VHVProduct | null>(null);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/vhv/products/${editing.id}`, {
        method: "PUT",
  ...
  const handleDelete = async () => {
    ...
    setActionError(null);
    try {
      const res = await apiFetch(`/vhv/products/${deleting.id}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json();
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="food, electronics, transport..."
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                      ...
                      <span className="text-white font-semibold">{deleting.name || `el producto #${deleting.id}`}</span>?
                      Se borrará también su historial de cálculos. Esta acción no se puede deshacer.
                    </p>
                  </div>
                </div>
              </div>
