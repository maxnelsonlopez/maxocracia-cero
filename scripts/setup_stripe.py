...
"""
    ...
    if current_contributor and not current_contributor.startswith("..."):
        ...
        if confirm("¿Actualizar?"):
            env_vars["STRIPE_PRICE_CONTRIBUTOR"] = (
                get_input("STRIPE_PRICE_CONTRIBUTOR", required=False)
    ...
    if current_enterprise and not current_enterprise.startswith("..."):
        ...
        if confirm("¿Actualizar?"):
            env_vars["STRIPE_PRICE_ENTERPRISE"] = (
                get_input("STRIPE_PRICE_ENTERPRISE", required=False)
...
"""
    ...
    if current_wh and not current_wh.startswith("..."):
        print("Webhook secret actual configurado")
        if confirm("¿Actualizar?"):
