...
def build_frontend():
    ...
    try:
        ...
        subprocess.run(
            ["xcopy", "/E", "/I", "/Y", "frontend\\out\\*", "app\\static\\dist\\"],
            cwd=base_dir,
            check=True,
            shell=True,
        )
        ...
        print(
            f"INFO: Generados {twins} payloads RSC en forma de puntos (navegación cliente)."
        )
        ...
        missing = [
            ...
            if not os.path.exists(os.path.join(static_dist_dir, f))
        ]

        if not missing:
            print(
                f"OK: Build completado y verificado exitosamente en {static_dist_dir}"
            )
        else:
            print(
                f"AVISO: El build terminó pero faltan archivos críticos: {', '.join(missing)}"
            )
