...
def check_dependencies():
    ...
    for package in required:
        try:
            ...
            check_colored_print(
                f"Dependencia: {package}", "error", f"Ejecuta: pip install {package}"
            )
            all_ok = False
...
def check_env_file():
    ...
    if not env_path.exists():
        if example_path.exists():
            check_colored_print(
                "Archivo .env", "error", "No existe. Copia: cp config.example.env .env"
            )
        else:
            check_colored_print(
                "Archivo .env", "error", "No existe config.example.env para copiar"
            )
        return False
...
def check_stripe_config():
    ...
    for var in required_vars:
        ...
        if not value or value.startswith("...") or value == f"{var.lower()}_here":
            check_colored_print(
                f"Variable {var}", "error", "No configurada o valor placeholder"
            )
            all_ok = False
        else:
...
def check_database():
    ...
    try:
        ...
        check_colored_print("Base de datos", "error", str(e))
        return False
...
def check_frontend():
    ...
    if not frontend_path.exists():
        check_colored_print("Frontend", "error", "No se encontró frontend/package.json")
        return False
    ...
    if not node_modules.exists():
        check_colored_print(
            "Frontend dependencias", "warning", "Ejecuta: cd frontend && npm install"
        )
        return False
...
def check_migrations():
    ...
    try:
        ...
        if result:
            ...
            required_columns = ["user_id", "tier", "status", "external_customer_id"]
            ...
            for col in required_columns:
                if col in column_names:
                    ...
                    check_colored_print(
                        f"  Columna: {col}", "error", "Falta columna requerida"
                    )
        ...
        check_colored_print("Migraciones", "error", str(e))
        return False
...
def main():
    ...
    if passed == total:
        print()
        check_colored_print("🎉 Todo está configurado correctamente", "ok")
        check_colored_print("Puedes iniciar el servidor: python run.py", "info")
        return 0
    else:
        print()
        check_colored_print("⚠️  Hay problemas que debes solucionar", "warning")
        check_colored_print("Revisa los errores arriba y corrígelos", "info")
        return 1
